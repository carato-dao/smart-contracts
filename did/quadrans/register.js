...
const CONTRACT_ADDRESS = process.env.QUADRANS_CONTRACT;
const OWNER_ADDRESS = process.env.QUADRANS_OWNER;
...
async function main() {
  ...
  try {
    ...
    const random = strings.random(66)
    console.log('--')
    const card_hash = crypto.createHash('sha256').update(random).digest('hex')
    ...
    await contract.methods.registerIdentity(identity, public_hash).send({ from: OWNER_ADDRESS })
    // Now checking if eID is stored correctly
    const stored = await contract.methods.returnEid(public_hash).call()
    if (identity === stored) {
